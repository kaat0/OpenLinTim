...
public class CreateLinepool {
	...
	public static void main(String[] args) throws IllegalAccessException, InstantiationException,
		...
		try {
			...
			boolean add_shortest_paths = config.getBooleanValue("lpool_add_shortest_paths");
			String lc_solver_name = config.getStringValue("lc_solver");
			double ratio_sp;
			if(add_shortest_paths){
				ratio_sp = config.getDoubleValue("lpool_ratio_shortest_paths");
			}
			else{
				ratio_sp=0;
			}
			...
			while (count < max_iterations && !feasible) {
				...
				// Else: Preference by Line-Concept if not yet feasible
					...
					if (feasible) {
						/*if(add_shortest_paths){
							System.err.print("\tCreate lines as shortest paths...");
								pool.addKSPLines(od, ratio_sp, 1);
							System.err.println("done!");
						}
