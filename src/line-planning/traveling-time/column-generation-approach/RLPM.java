...
//---------------Constructor-----------------------------
	/** Constructor sets up restricted linear programming master
	  * @param  Array List containing ArrayLists of paths for each od pair
	  * @param 	pool containing the Lines
	  * @param 	cag corresponding change and go graph
	  */
	
	public RLPM(ArrayList<ArrayList<Path>> paths, Pool pool, CAG cag) {
		...
		try{
			...
			this.RELAXATION = config.getIntegerValue("lc_traveling_time_cg_constraint_type");
			this.BUDGET = config.getDoubleValue("lc_budget");
			this.RELAXATION_CONSTRAINT = config.getBooleanValue("lc_traveling_time_cg_relaxation_constraint");
			this.PRINT_PATH_VAR = config.getBooleanValue("lc_traveling_time_cg_print_path_variables");
			this.VERBOSE = config.getBooleanValue("lc_verbose");
		}catch(IOException e){
			System.out.println("Cannot get parameter from config");
		}
		...
		x = new ArrayList<ArrayList<XPRBvar>>(0);
		y = new ArrayList<XPRBvar>(0);
		
		for (int i = 0; i<paths.size(); i++){
			x.add(new ArrayList<XPRBvar>(0));
		...
		odConstr = new ArrayList<XPRBctr>(0);
		for(int i = 0; i < paths.size(); i++){
		...
		if (RELAXATION == 1){
			lineConstr1 = new ArrayList<XPRBctr>(0);
		...
		//third variant: sum up constraints over all line edges corresponding to one line
			...
			lineConstr3 = new ArrayList<ArrayList<XPRBctr>>(0);
			ArrayList<Integer> lineList;
			//initialization of constraint and add y variables
			for(int j = 0; j < paths.size(); j++){
				lineConstr3.add(new ArrayList<XPRBctr>(0));
				for(int i = 0; i < pool.getLines().size(); i++){
		...
		//fourth variant: one constraint for every x_p, y_l with l used by p
			...
			lineConstr4 = new ArrayList<ArrayList<ArrayList<XPRBctr>>>(0);
			ArrayList<Node> nodeList;
			//initialisation of constraint and add y variables
			// one constraint for each line
			for(int k = 0; k < x.size(); k++){
				lineConstr4.add(new ArrayList<ArrayList<XPRBctr>>(0));
				for(int j = 0; j < cag.getLineEdges().size(); j++){
					lineConstr4.get(k).add(new ArrayList<XPRBctr>(0));
					for(int i = 0; i < cag.getLineEdges().get(j).size(); i++){
		...
		if(RELAXATION_CONSTRAINT){
			relaxConstr = new ArrayList<XPRBctr>(0);
...
//-------------getPosition-------getNumberOfOdPairs---------------------------------
	...
	/** @param two ids of two different od nodes
