...
void PassengerTest::testAdvance(){
	...
	act.push_back(act2);
	Path path(act, 0);
	Passenger * pass = new Passenger(repast::AgentId(1,0,1,0), path,act1,3,ONLINE);
	context.addAgent(pass);
	act1->addAgent(pass);
	pass->advance();
	std::unordered_set<Passenger*> agentsOnAct2 = act2->local_agents();
	ASSERT_EQUAL(0, act1->local_agents().size());
	ASSERT_EQUAL(1, act2->local_agents().size());
	Passenger * same_pass = *agentsOnAct2.begin();
