...
public class LinePool {
	...
	//covered
	private static int min_cover_factor;
	...
	public static void setMinCoverFactor(int n) {
		min_cover_factor = n;
	}
...
//Fix terminals-----------------------------------------------------------
	...
	private int lineBasedToTerminal() {
		...
		if(pool.isEmpty()){
			return ptn.stopsToTerminal(node_degree_ratio);
		}

		int max_deg = 0;
		int sum = 0;
		HashMap<Stop, Integer> number_of_lines = new HashMap<Stop, Integer>();
		for (Stop stop : ptn.getStops()) {
	...
	public boolean preferProblematicEdges(int[] line_concept){
		...
		Hashtable<Edge, Integer> load=new Hashtable<Edge,Integer>();
	...
	public void finalizeSP() {
		// Delete double entries
		Collections.sort(pool);
		Line outer_line = null;
		Line inner_line = null;
