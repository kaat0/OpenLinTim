...
public class DSLTT2{
	...
	private void constructXpressIPFormulation(FiniteDominatingSet fds, CandidateEdgeSet ecs, Demand demand, int waitingTime, Distance distance){
        ...
        XPRBctr[] ctrBuildingLeftPos = new XPRBctr[number_candidates];
        XPRBctr[] ctrBuildingRightPos = new XPRBctr[number_candidates];
			...
			if(candidate.isVertex()){
				for(Edge edge:candidate.getEdges()){
					if(distance.calcDist(edge.getLeft_stop(),candidate)<Distance.EPSILON){
						...
						/*ctrBuildingLeftPos[indexMap.get(""+candidate.getId()+"."+edge.getIndex())] = p.newCtr("building_left_pos_"+candidate.getId()+"."+edge.getIndex());
						ctrBuildingLeftPos[indexMap.get(""+candidate.getId()+"."+edge.getIndex())].setType(XPRB.G);
						ctrBuildingLeftPos[indexMap.get(""+candidate.getId()+"."+edge.getIndex())].setTerm(x[candidate.getId()-1],1.0);*/
					} else {
						...
						/*ctrBuildingRightPos[indexMap.get(""+candidate.getId()+"."+edge.getIndex())] = p.newCtr("building_right_neg_"+candidate.getId()+"."+edge.getIndex());
						ctrBuildingRightPos[indexMap.get(""+candidate.getId()+"."+edge.getIndex())].setType(XPRB.G);
						ctrBuildingRightPos[indexMap.get(""+candidate.getId()+"."+edge.getIndex())].setTerm(x[candidate.getId()-1],1.0);*/
					}
				}
			} else {
				...
				/*ctrBuildingLeftPos[indexMap.get(""+candidate.getId()+"."+candidate.getEdges().get(0).getIndex())] = p.newCtr("building_left_pos_"+candidate.getId()+"."+candidate.getEdges().get(0).getIndex());
				ctrBuildingRightPos[indexMap.get(""+candidate.getId()+"."+candidate.getEdges().get(0).getIndex())] = p.newCtr("building_right_pos"+candidate.getId()+"."+candidate.getEdges().get(0).getIndex());	
				ctrBuildingLeftPos[indexMap.get(""+candidate.getId()+"."+candidate.getEdges().get(0).getIndex())].setType(XPRB.G);
				ctrBuildingLeftPos[indexMap.get(""+candidate.getId()+"."+candidate.getEdges().get(0).getIndex())].setTerm(x[candidate.getId()-1],1.0);
				ctrBuildingRightPos[indexMap.get(""+candidate.getId()+"."+candidate.getEdges().get(0).getIndex())].setType(XPRB.G);
				ctrBuildingRightPos[indexMap.get(""+candidate.getId()+"."+candidate.getEdges().get(0).getIndex())].setTerm(x[candidate.getId()-1],1.0);*/
			}
		}
			...
			if(candEdge.getLeft_candidate().isVertex()){
				for(Edge edge:candEdge.getLeft_candidate().getEdges())
					if(candEdge.getOriginal_edge()==edge){
						...
						//ctrBuildingLeftPos[indexMap.get(""+candEdge.getLeft_candidate().getId()+"."+edge.getIndex())].setTerm(y[candEdge.getIndex()-1],-1.0);
					}
			} else {
				...
				//ctrBuildingLeftPos[indexMap.get(""+candEdge.getLeft_candidate().getId()+"."+candEdge.getLeft_candidate().getEdges().get(0).getIndex())].setTerm(y[candEdge.getIndex()-1],-1.0);
			}
			if(candEdge.getRight_candidate().isVertex()){
				for(Edge edge:candEdge.getRight_candidate().getEdges())
					if(candEdge.getOriginal_edge()==edge){
						...
						//ctrBuildingRightPos[indexMap.get(""+candEdge.getRight_candidate().getId()+"."+edge.getIndex())].setTerm(y[candEdge.getIndex()-1],-1.0);
					}
			} else {
				...
				//ctrBuildingRightPos[indexMap.get(""+candEdge.getRight_candidate().getId()+"."+candEdge.getRight_candidate().getEdges().get(0).getIndex())].setTerm(y[candEdge.getIndex()-1],-1.0);
			}
	...
	public void solve(){
		...
		for(Candidate candidate:fds.getCandidates()){
			/*for(Edge edge:candidate.getEdges()){
				indexMap.put(""+candidate.getId()+"."+edge.getIndex(),candidate_it);
				x_sol[candidate_it] = p.getVarByName("x_"+candidate.getId()+"."+edge.getIndex()).getSol()==1.0?true:false;
			}*/
			//System.out.println("x_"+candidate.getId() + " , " + p.getVarByName("x_"+candidate.getId()).getSol());
			x_sol[candidate.getId()-1] = Math.round(p.getVarByName("x_"+candidate.getId()).getSol())==1?true:false;
		...
		for(CandidateEdge candidateEdge:ecs.getCandidateEdges()){
			//System.out.println("y_"+candidateEdge.getIndex() + ","+candidateEdge.getLeft_candidate().getId()+"_"+candidateEdge.getRight_candidate().getId() + " , " + p.getVarByName("y_"+candidateEdge.getIndex()).getSol());
