...
public class IO
{
    ...
    private static String vehicleFlowFileName = "vehicle-scheduling/Vehicle_Flow.vs";

    // default names for output files which are only generated when "vs_verbose" is true
    private static String canalNetworkFileName = "vehicle-scheduling/Canal_Network.vs";
        ...
        private static String transfersFileName = "vehicle-scheduling/Transfers.vs";
    ...
    private static int minDistance = 60; // This is the minimal distance (in seconds) between two trips, i.e. a train has to stay this time in a station.
    private static int penCosts = 50000; // This are the costs, which occurs, when one trip isn't served -> FIXME: different penalty costs for each trip?
    ...
    private static boolean ptnIsUndirected = true; // Whether or not the PTN is undirected
    ...
    private static String stationDistancesFileName = null;
    ...
    public static ArrayList<CTrip> readCTrips() throws IOException {
        ...
        String line;
        ArrayList<CTrip> trips = new ArrayList<CTrip>();
     ...
     *
     * @returns events, an ArrayList<Integer>, which contains the id's of the
     *          occuring events
     */
     ...
     *
     * @returns transfers, an ArrayList<CTransfer>, which contains the
     ...
     * </ul>
     * @returns transfers, an ArrayList<Transfer>, which contains the transfers, each transfer is represented by
      * 		<ul>
      * 		<li> ID: unique ID for the trip
     ...
     */
        ...
        while ((line = reader.readLine()) != null)
        {
            ...
            if(models.get(modelName) == 2){
                ...
                Transfer transfer = new Transfer(ID, firstTripID, secondTripID, timeCycleJump);
                transfers.add(transfer);

            }
            else{
                Transfer transfer = new Transfer(ID, firstTripID, secondTripID);
                transfers.add(transfer);
            }
     ...
     * @return edges, an ArrayList<Edge>, which contains the edges of the network, each edge is represented by
     ...
     */
        ...
        String[] entries = line.split(";");
            int edgeID = Integer.parseInt(entries[0].trim());
...
//							                   // 	=> length in seconds = length in meter * (3600/1000) * (1/speed) s/m = length in meter * (3600/speed) * (1/1000) (s/m)
        Edge edge = new Edge(edgeID, leftStopID, rightStopID,
                lowerBound*timeUnitsPerMinute*60);
...
//                		(int) (length_in_seconds * 100));
    ...
    public static void calculateMoselInput() throws FileNotFoundException, IOException{
        Map<String, Integer> models = new HashMap<String, Integer>();
        models.put("MDM1", 0);
        models.put("MDM2", 1);
        models.put("ASSIGNMENT_MODEL", 2);
        ...
        if(modelName.toUpperCase().equals("CANAL_MODEL")){
            ...
            double[] penaltyCosts = new double[trips.size()];
            for(int i = 0; i < trips.size(); i++){
                penaltyCosts[i] = 50000; //FIXME: What value to take here? Global variables?
            }
    ...
    public static void calculateMoselInputVehicleFlow(CanalNetwork network) throws FileNotFoundException, IOException{
    ...
    for(int i = 0; i < transfers.size(); i++){
        transferString.append(Integer.toString(transfers.get(i).getCosts()));
    ...
    for(Canal canal: network.getCanals()) {
        ...
        for(CEvent event: canal.getEvents()) {
            ...
            eventTypeString.append(event.getType() == Event.TYPE_START? " 0": " 1");
        }
    }
    ...
    Set<Integer> startIDs = new HashSet<Integer>();
    ...
    for(int i = 0; i < timeOfTransfers.length; i++){
        timeOfTransfers[i] = -1;
    }
    for(int i = 0; i < transfers.size(); i++){
        timeOfTransfers[transfers.get(i).getStartEvent().getID()-1] = transfers.get(i).getStartEvent().getTime();
        timeOfTransfers[transfers.get(i).getEndEvent().getID()-1] = transfers.get(i).getEndEvent().getTime();
    }
    for(int i = 0; i < timeOfTransfers.length; i++){
        EventTimeString.append(timeOfTransfers[i]);
    ...
    public static void calculateMoselInputForMDM1(ArrayList<Trip> trips) throws FileNotFoundException{
        ...
        for(int i = 0; i <= trips.size(); i++){
            for(int j = 0; j <= trips.size(); j++){
                ...
                else if((trips.get(i-1).getEndStation() == trips.get(j-1).getStartStation()) && (trips.get(i-1).getEndTime() + minDistance <= trips.get(j-1).getStartTime())){					if(j < trips.size()){
                        pstream.print(" 0,");
    ...
    public static void calculateMoselInputForMDM2(ArrayList<Trip> trips) throws FileNotFoundException{
        ...
        for(int i = 0; i < trips.size(); i++){
            for(int j = 0; j < trips.size(); j++){
                if((trips.get(i).getEndStation() == trips.get(j).getStartStation()) && (trips.get(i).getEndTime() + minDistance <= trips.get(j).getStartTime())){
                    if(j < trips.size()-1){
                        pstream.print(" 1,");
    ...
    public static void calculateVSFile(ArrayList<Trip> trips, ArrayList<Transfer> transfers) throws FileNotFoundException, IOException{
        Map<String, Integer> models = new HashMap<String, Integer>();
        models.put("MDM1", 0);
        models.put("MDM2", 1);
        models.put("ASSIGNMENT_MODEL", 2);
    ...
    public static void calculateVSFileMDM1(ArrayList<Trip> trips, ArrayList<Transfer> transfers) throws FileNotFoundException{
        ...
        for(int i = 0; i < transfers.size(); i++){
            transfersMatrix[transfers.get(i).getFirstTripID()][transfers.get(i).getSecondTripID()] = true;
        }
    ...
    public static void calculateVSFileMDM2(ArrayList<Trip> trips, ArrayList<Transfer> transfers) throws FileNotFoundException{
        ...
        for(int i = 0; i < transfers.size(); i++){
            transfersMatrix[transfers.get(i).getFirstTripID()-1][transfers.get(i).getSecondTripID()-1] = true;
        }
    ...
    public static void checkAssignments(ArrayList<Trip> trips, ArrayList<Transfer> transfers) throws IOException{
        boolean allright = true;

        int[] numberOfLeftDepartures = new int[stops.size()];
        for(int i = 0; i < numberOfLeftDepartures.length; i++){
            numberOfLeftDepartures[i] = 0;
        }
        for(int i = 0; i < transfers.size(); i++){
            numberOfLeftDepartures[trips.get(transfers.get(i).getFirstTripID()-1).getEndStation()-1]++;
            numberOfLeftDepartures[trips.get(transfers.get(i).getSecondTripID()-1).getStartStation()-1]--;
        }

        for(int i = 0; i < trips.size(); i++){
            numberOfLeftDepartures[trips.get(i).getStartStation()-1]++;
            numberOfLeftDepartures[trips.get(i).getEndStation()-1]--;
        }
        ...
        int minDistanceBetweenTwoTripsAtTheSameStation = 86400;

        for(int i = 0; i < transfers.size(); i++){
            if(trips.get(transfers.get(i).getFirstTripID()-1).getEndStation() == trips.get(transfers.get(i).getSecondTripID()-1).getStartStation()){
                if(minDistanceBetweenTwoTripsAtTheSameStation > (trips.get(transfers.get(i).getSecondTripID()-1).getStartTime() - trips.get(transfers.get(i).getFirstTripID()-1).getEndTime()
                   + 86400) % 86400){
                    minDistanceBetweenTwoTripsAtTheSameStation = (trips.get(transfers.get(i).getSecondTripID()-1).getStartTime() - trips.get(transfers.get(i).getFirstTripID()-1).getEndTime()
                    + 86400)
                    % 86400;
                }
            }
        ...
        int minDistanceBetweenTwoTripsDifferentStations = 86400;

        for(int i = 0; i < transfers.size(); i++){
            if(trips.get(transfers.get(i).getFirstTripID()-1).getEndStation() != trips.get(transfers.get(i).getSecondTripID()-1).getStartStation()){
                if(minDistanceBetweenTwoTripsDifferentStations > (trips.get(transfers.get(i).getSecondTripID()-1).getStartTime() - trips.get(transfers.get(i).getFirstTripID()-1).getEndTime()
                   + 86400) % 86400){
                    minDistanceBetweenTwoTripsDifferentStations = (trips.get(transfers.get(i).getSecondTripID()-1).getStartTime() - trips.get(transfers.get(i).getFirstTripID()-1).getEndTime()
                    + 86400) % 86400;
                }
            }
        }
     ...
     * {@link #readTransfers}
     */
    public static void calculateVSFileAM(ArrayList<Trip> trips, ArrayList<Transfer> transfers) throws FileNotFoundException, IOException{
        ...
        for(int i = 0; i < transfers.size(); i++){
            if(transfers.get(i).getValueTimeCycleJump()){
                numberOfVehicles++;
            }
        }
        ...
        for(int i = 0; i < transfers.size(); i++){
            if(! transfers.get(i).getValueTimeCycleJump()){
                transfersMatrix[transfers.get(i).getFirstTripID()-1][transfers.get(i).getSecondTripID()-1] = 1;
    ...
    public static CanalNetwork calculateCanalNetwork() throws IOException{
        ...
        // FIXME: Read "parkingAllowed" and "maintainingAllowed" out of the config
        boolean[] parkingAllowed = new boolean[stops.size()];
        boolean[] maintainingAllowed = new boolean[stops.size()];

        for(int i = 0; i < stops.size();  i++){
            parkingAllowed[i] = false;
            maintainingAllowed[i] = false;
        }

        network.setTrips(trips);

        network = VS.calculateCanalsAndTransfers(network, stops, parkingAllowed,
                maintainingAllowed, vehicleCosts, distances, turnOverTime,
                ptnIsUndirected, minDistance);

        return network;
    }

    public static void checkAssignments(ArrayList<CTrip> trips, ArrayList<CTransfer> transfers, HashMap<Canal, HashMap<CEvent, CEvent>> mappings, int[][] transfersMatrix){
                    ...
                    if(transfersMatrix[i][j] != 0){
                        if(trips.get(i).getEndStation() == trips.get(j).getStartStation()){
                            if(minDistanceBetweenTwoTripsAtTheSameStation >
                                    (trips.get(j).getStartTime() - trips.get(i).getEndTime() + 86400) % 86400){
                                minDistanceBetweenTwoTripsAtTheSameStation = (trips.get(j).getStartTime() - trips.get(i).getEndTime() + 86400) % 86400;
                                }
                        }
                    }
                }
            ...
            int minDistanceBetweenTwoTripsAtDifferentStations = 86400;
                    ...
                    if(transfersMatrix[i][j] != 0){
                        if(trips.get(i).getEndStation() != trips.get(j).getStartStation()){
                            if(minDistanceBetweenTwoTripsAtDifferentStations >
                                    (trips.get(j).getStartTime() - trips.get(i).getEndTime() + 86400) % 86400){
                                minDistanceBetweenTwoTripsAtDifferentStations = (trips.get(j).getStartTime() - trips.get(i).getEndTime() + 86400) % 86400;
                                }
                        }
                    }
                }
    ...
    throws FileNotFoundException, IOException{
        ...
        HashMap<Integer,Integer> eventsOfTheLines = new HashMap<Integer, Integer>();
        HashMap<Integer,Integer> eventsOfTheOccuringTransfers = new HashMap<Integer, Integer>();
        HashMap<Integer,Integer> eventsOfTheMappings = new HashMap<Integer, Integer>();
        HashMap<Integer,Integer> lineIDforGivenEventIDs = new HashMap<Integer, Integer>();

        HashMap<Integer,CTransfer> transferForGivenEventIDs = new HashMap<Integer, CTransfer>();
        ...
        // This matrix shows, if trip j is served directly after trip i by the same vehicle. Only in this case, transfersMatrix[i][j] will be true.
        int[][] transfersMatrix = VS.calculateTransfersMatrix(stops, trips,
                lineIDforGivenEventIDs, eventsOfTheLines,
                eventsOfTheOccuringTransfers, eventsOfTheMappings,
                greatestLineID, distances, transferForGivenEventIDs, minDistance);

        checkAssignments(trips, occuringTransfers, mappings, transfersMatrix);
    ...
    public static void initialize(Config config) throws IOException, Exception{

        modelName = config.getStringValue("vs_model").toUpperCase();
        vehicleCosts = (int)(config.getDoubleValue("vs_vehicle_costs")*1);
        minDistance = config.getIntegerValue("vs_min_distance");
        turnOverTime = config.getIntegerValue("vs_turn_over_time");
        verbose = config.getBooleanValue("vs_verbose");
        ptnIsUndirected = config.getBooleanValue("ptn_is_undirected");
        ...
        stationDistancesFileName = config.getStringValue("default_vs_station_distances_file");
