...
int curr_length;
std::vector<std::vector<int> > Cost_Matrix;     // Matrix of Costs for each matching
std::vector<std::vector<int> > matching_list;   // output of matching
...
vector<int> mate (MaxNX+1);
vector<int> lab (MaxNX + 1);
vector<int> q (MaxN);
vector<int> fa (MaxNX + 1);
vector<int> col (MaxNX + 1);
vector<int> slackv (MaxNX + 1);
vector<int> bel (MaxNX + 1);
vector<int> bloch [MaxNX + 1];
int q_n;

int n_x;
vector<vector<int> > blofrom (MaxNX + 1);

void init(){
    
    for (int i = 0; i < MaxNX + 1; i++)
    {
        blofrom[i].resize(MaxNX + 1);
    }
}
...
int chinese_matching()
{
    // main
    init();
    n = curr_length, m = curr_length;
    
    int maximum = 0;
    
    for (int v = 0; v < curr_length - 1; v++)
    {
        for (int u = v + 1; u < curr_length; u++)
        {
            if (Cost_Matrix[v][u] > maximum)
            {
                maximum = Cost_Matrix[v][u];
            }
        }
    }
    for (int v = 0; v < curr_length; v++)
    {
        for (int u = 0; u < curr_length; u++)
        {
            Cost_Matrix[v][u] = maximum + 1 - Cost_Matrix[v][u];
        }
    }
    for (int v = 0; v < curr_length; v++) {
        Cost_Matrix[v][v] = 0;
    ...
    for (int v = 1; v <= n; v++)
    {
        for (int u = 1; u <= n; u++)
        {
            mat[v][u] = edge(v, u, 0);
            mat[u][v] = edge(u, v, 0);
            mat[v][u].w = mat[u][v].w = Cost_Matrix[u-1][v-1];
        }
    }
    ...
    for (int v = 0; v < curr_length; v++) {
        int unused = 1;
        for (int dummy = 0; dummy < u; dummy++) {
            if (v == matching_list[dummy][0] || v == matching_list[dummy][1]) {
                unused = 0;
            }
        }
        if (unused && u < (int) curr_length/2 && mate[v+1] != 0) {
            matching_list[u][0] = v;
            matching_list[u][1] = mate[v+1]-1;
            u++;
        }
    }
